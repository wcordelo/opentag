...
| Source-scoped memory deletion receipts | `edge/src/platform/layer3-contract.ts`, `platform-state-do.ts` | Durable epoch-bound proof ledger before deletion is marked complete | Implemented locally; external deletion executor and source-by-source provider calls remain gated |
| Receipt-bound tenant provisioning | `edge/src/platform/layer3-contract.ts`, `platform-state-do.ts`, `provisioning-contract.ts`, `workers/provisioning-adapter/` | Durable required-step evidence plus a fail-closed step-scoped bootstrap boundary before tenant activation | Adapter boundary implemented and tested; tenant locator, resource authority, Slack install, custody, and live provisioning remain gated |
