...
| Immutable connector labels and credential references | `edge/src/connectors/authorization.ts`, `authorization-snapshot.ts`, `platform-authorization.ts`, `credential-broker.ts`, `WorkspaceConfigDO`, `PlatformStateDO` | Cloudflare-native authorization boundary | Implemented with bundle plus server-owned OAuth/marketplace/custody snapshot fences; verified Slack ingress evidence is preserved through normal/deferred turns; tokens remain outside OpenTag state |
| Versioned billing entitlement gate | `edge/src/platform/layer3-contract.ts`, `platform-state-do.ts` | Durable tenant plan/period/limit checks before usage receipts | Implemented locally; payment provider, invoicing, and reconciliation adapter remain gated |
| Receipt-bound tenant provisioning | `edge/src/platform/layer3-contract.ts`, `platform-state-do.ts`, `tenant-locator.ts`, `provisioning-contract.ts`, `workers/provisioning-adapter/` | Durable server-owned external→internal locator, required-step evidence, and a fail-closed step-scoped bootstrap boundary before tenant activation | Locator registry/resolver and provisioning integration are source-complete and tested; bootstrap authority, resource receipts, Slack install, custody, and live provisioning remain gated |
| Server-owned identity links | `edge/src/platform/identity-link.ts`, `platform-state-do.ts`, `slack-v1-adapter.ts` | Tenant-scoped external-subject to principal binding with proof/version/revocation fences | Source-complete and focused-tested; proof issuer, key custody, and live identity installation remain gated |
...
| Last-mile credential broker | `edge/workers/credential-broker/`, `edge/src/connectors/credential-broker.ts`, `edge/src/connectors/authorization-snapshot.ts` | Revalidates server-owned OAuth grant, curated marketplace, tenant/provider/scope metadata before an external custody binding | Implemented locally; custody adapter, identity-bound runtime wiring, and provider tokens remain unconfigured |
