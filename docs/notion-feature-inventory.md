...
mapping from daily Centaur findings to OpenTag decisions. The OAuth/marketplace
branch adds a replay-safe state store, durable trust/version gates, and an
authenticated provider-adapter protocol, but does not claim a provider
callback, token exchange, or custody deployment.
