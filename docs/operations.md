...
| Work | Repository |
| --- | --- |
| Features, tests, local `wrangler dev`, CI | **opentag** |
| Worker vars/secrets, staging/preview/prod deploy, enterprise-only code | **Downstream deployment repo** |

Cloudflare deploy scripts in opentag are blocked (exit non-zero, no wrangler
call). `wrangler dev` and the CI test sequence still run here.
...
| `KNOWLEDGE_RERANK_MODE` | Var | Bot | Optional Jev rerank for knowledge search: `off` (default), `jev-score`, or `jev-noul`; falls back to RRF order on error. Production var provisioning is in the downstream deployment repo |
...
| `TYPESAFE_API_KEY` | Secret | Bot | TypeSafe API key for Jev reranking; never logged or exposed to callers. Set in the downstream deployment repo before enabling rerank mode |
...
5. Deploy the harness from the downstream deployment repo, then deploy the bot
   (see [Deploy the bot](#deploy-the-bot)):

```bash
# upstream: develop and test locally only
cd edge/workers/sandbox
npm run typecheck
```

Harness auth and bot binding configuration are deployed from the downstream repo.

For a new installation, the one-command installer in opentag can still validate
secrets locally with `--no-deploy`, but any path that would call wrangler exits
non-zero in this repo. Finish deploy in the downstream deployment repo:
