...
function lexicalRank(query, docs, limit) {
  ...
  const lengths = docs.map((doc) => tokenize(doc.excerpt).length);
  const avgLen = lengths.reduce((sum, len) => sum + len, 0) / Math.max(1, docs.length);
  const docFreq = new Map();
  for (const doc of docs) {
    const seen = new Set(tokenize(doc.excerpt));
    for (const term of seen) docFreq.set(term, (docFreq.get(term) ?? 0) + 1);
  }
  const scored = docs.map((doc) => ({
    ...doc,
    score: bm25Score(queryTokens, tokenize(doc.excerpt), avgLen, docFreq, docs.length),
  }));
...
function wikiDocsFromMarkdown(filePath, pageId) {
  ...
  const docs = [{ id: pageKey, excerpt: `# ${title}\n\n${body.slice(0, 1200)}`, sourceFamily: "wiki" }];
...
const QUERY_SPECS = [
  ...
  { query: "parallel multi-list search fused with RRF", gold: "code:T_eval:opentag:src/memory/retrieval/unified-search.ts:52-103", family: "code" },
  ...
  { query: "KnowledgeCitationBase excerpt contentRevision", gold: "code:T_eval:opentag:src/memory/knowledge-contract.ts:26-184", family: "code" },
