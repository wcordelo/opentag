...
/**
 * The StateStore conformance suite, vendored verbatim from `@copilotkit/bot`
 * (`src/testing/state-store-conformance.ts`). Running our SQLite engine through
 * it proves byte-for-byte behavioral parity with the in-memory reference store.
 * Keep in sync with upstream; once `@copilotkit/bot` is installed, import
 * `runStateStoreConformance` from it instead.
 */
