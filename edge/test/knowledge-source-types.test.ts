...
describe("knowledge source types (K2)", () => {
  it("parses the supported sourceType values", () => {
    ...
    expect(parseKnowledgeSourceType("drive")).toBe("drive");
  ...
  it("builds wiki/code/custom_db source keys with Slack-consistent validation", () => {
    ...
    expect(driveSourceKey("T1", "P1", "file-123")).toBe("drive:T1:P1:file-123");
  ...
  it("derives sourceType from sourceKey prefixes including custom_db", () => {
    ...
    expect(parseSourceTypeFromKey("drive:T1:P1:file")).toBe("drive");
  ...
  it("marks wiki/code/custom_db/drive connectors implemented in Phase 2", () => {
    ...
    expect(CONNECTOR_REGISTRY.drive?.implemented).toBe(true);
  });
});
