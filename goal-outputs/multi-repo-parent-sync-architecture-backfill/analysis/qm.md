...
>
> The QM report's OpenTag comparison predates the merged platform/router work and the live rollout. Current status: QM capability/serviceability/typed-provenance lessons are adapted; native Nanocodex, runtime evidence, actor-bound knowledge controls, router shadow, and platform metadata are now present. Worker Secrets remain deployment/bootstrap configuration, not per-tenant custody; the broker/effecter is still open.
