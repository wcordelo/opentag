...
([`PRODUCT.md:50-67`](../../docs/PRODUCT.md), [`DECISIONS.md:49-56`](../../docs/DECISIONS.md)).
...
| Slack events are acknowledged then continued in `waitUntil`, and pre-admission occurs before async work. | [`ARCHITECTURE.md:85-102`](../../docs/ARCHITECTURE.md), [`PRODUCT.md:115-125`](../../docs/PRODUCT.md). | The only compatible event hook is `waitUntil -> KnowledgeDO -> Queue`; it must be nonblocking and must not change turn success/failure or Slack acknowledgement. |
...
| The bot’s state/control invariants are strict. | Stable IDs/pre-admission are locked in [`DECISIONS.md:149-179`](../../docs/DECISIONS.md); state is divided among conversation delivery, session events, and runtime ([`ARCHITECTURE.md:170-177`](../../docs/ARCHITECTURE.md)). | Knowledge jobs must have their own stable IDs and ledger; do not overload active-turn IDs, session events, or turn effect fences for background corpus lifecycle. |
